...
int main(){
    ...
    vet_count = malloc(LETRAS * sizeof(unsigned long int));
    arquivo = stdin;
...
wchar_t * existe(wint_t * linha){
	...
	pthread_mutex_lock(&mutex_arquivo);
        retorno = fread_unlocked(linha, sizeof(wchar_t), TAMANHO_BUFFER - 1, arquivo); 
...
void *le_arquivo(void *id){
	...
	memset(aux, 0, LETRAS * sizeof(unsigned long int));	
